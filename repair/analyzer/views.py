...
def table_1_show(request):
    ...
    # f = open(os.path.join(settings.BASE_DIR,'\\Study\\IGI\pythn_igi\\Lab_4\\clean\\analyzer\\templates\\tables.html'), "w")
    # f.write(str)
    # f.close()
...
def predict_show(request, id):
    ...
    title = Usluga.objects.get(id=id).title

    ords = list(OrderItem.objects.all())
    date_arr = list()
    for tmp in ords:
        if tmp.usluga.title == title:
...
def stupid_table(request):
    ...
    for tmp in ords:
        out_dict[tmp.usluga.genre] = dict()

    for tmp in ords:
        out_dict[tmp.usluga.genre][tmp.order.created.date().strftime("%Y-%m-%d")] = 0
        total[tmp.order.created.date().strftime("%Y-%m-%d")] = 0

    for tmp in ords:
        out_dict[tmp.usluga.genre][tmp.order.created.date().strftime("%Y-%m-%d")] += tmp.quantity
